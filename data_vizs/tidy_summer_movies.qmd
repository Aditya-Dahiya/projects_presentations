...
plotdf <- df |>
  ...
  relocate(id) |> 
  mutate(image_var = here::here("data_vizs", "temp_summer", paste0("image", id, ".png"))) |> 
  mutate(image_var = str_replace_all(image_var, "/", " \\"))
  # mutate(image_var = paste0("/data_vizs/temp_summer/image", id, ".png"))
```
...
download_movie_poster <- function(movie_name, num_row) {
  ...
  image_write(
    ...
    path = here::here("data_vizs", "temp_summer", 
                      paste0("image_", num_row,".png")),
    format = "png"
    )
}

for (i in 51:nrow(plotdf)) {
