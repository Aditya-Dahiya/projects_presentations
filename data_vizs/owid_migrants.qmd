...
top_flows <- plotdf1 |> 
  filter(colour_var == "Inter-Continental") |> 
...
plotdf2 |> 
  ...
  geom_text(
    data = plotdf2 |> filter(connector_var == "from"),
    mapping = aes(
      label = str_wrap(node_var, 10)),
    colour = "black",
    hjust = 1,
    position = sankey_pos(nudge_x = -0.06),
    stat = "sankeynode",
    lineheight = 0.5
    ) +
  # Right hand side labels
  geom_text(
    data = plotdf2 |> filter(connector_var == "to"),
    mapping = aes(
      label = str_wrap(node_var, 10)),
    colour = "black",
    hjust = 0,
