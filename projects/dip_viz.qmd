...
Lastly, the @fig-anim2-hdi shows another animation using {[ggflags](https://github.com/jimjam-slam/ggflags)} package by [James Goldie](https://github.com/jimjam-slam) on changing CO~2~ emissions compared with changing per capita national incomes from 1990 to 2022 for some countries. The annotation adds the message.
...
![An animated scatterplot of Gross National Income (x-axis) vs. Per-capita Carbon dioxide emissions (Y-axis) from year 1990-2022 for some countries. Incomes in Ireland and Luxembourg increased without raising emissions, while the emissions rose in Brunei and Qatar without much impact on per capita incomes.](/docs/hdi_anim_flags.gif){#fig-anim2-hdi}

### Using loess and purr::map to smoothen the animation

The @fig-anim3-hdi amends and improves the @fig-anim2-hdi making it smoother using the `loess` smoother.
